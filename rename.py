...
main_directory = "D:\Chuyen De Python\Code\Chuyen-De-Python"
...
for chapter_folder in os.listdir(main_directory):
    ...
    if os.path.isdir(chapter_path) and chapter_folder.startswith("Chuong"):
        # Extract the chapter number and format it
        chapter_number = chapter_folder.split("Chuong")[1].zfill(2)
        chapter = f"Chuong{chapter_number}"
        
        # Loop through each file in the chapter folder
        for filename in os.listdir(chapter_path):
            # Check if the file follows the pattern "cau X.py"
            if filename.startswith("cau") and filename.endswith(".py"):
                # Extract the cau number from the filename
                cau_number = filename.split(" ")[1].split(".")[0]
                # Format the cau number with leading zeros
                cau_number = cau_number.zfill(2)
                
                # Create the new filename with the specified format
                new_filename = f"{course_code}.{chapter}.cau{cau_number}.py"
